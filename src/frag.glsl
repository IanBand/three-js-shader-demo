...
void main() {
    ...
    distort.x = perlinNoise(vUv + vec2(0.0, time * -0.1 * time_dialation), scale,             seed) * strength;
    distort.y = perlinNoise(vUv + vec2(0.0, time * -1.6 * time_dialation), scale * 3.0, seed + 1.0) * strength * 1.3;
